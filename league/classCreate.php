	...
	<div class="new-wrap">
		<div class="top-title">
			<p>
				...
				<span class="top-t">新增团课开展情况-<?php echo $branch?></span>
			</p>
		</div>
    <div id="container">
...
$path = "../Files/".date("Y")."/".date("m")."/League/Class/";   // 文件存储路径
if (!is_dir($path)){
...
if (isset($_POST['year'])){
    // 上传文件
    if ($_FILES['file']['name'] != '') {
        $filename = "JB-Class-".date("Y-m")."-".$branch."-".$_FILES['file']['name'];    // 重命名文件
...
//        echo "<script>alert('请上传文件！');</script>";
    ...
    $query = "INSERT INTO `plans` (`ID`,`branch`, `year`, `month`, `season`, `fileName`, `type`) 
                  VALUES 
                  (NULL, '".$branch."', '".$_POST['year']."', NULL , '".$_POST['season']."', '".$path.$filename."', '团课')";
    $conn->query($query);
    $conn->close();
