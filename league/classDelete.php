	...
	<div class="new-wrap">
		<div class="top-title">
			<p>
				...
				<span class="top-t">删除团课开展情况-<?php echo $branch?></span>
			</p>
		</div>
    <div id="container">
        <div id="content">
...
if (isset($_POST['year'])){
    ...
    $query = "SELECT * FROM `plans` WHERE `branch` = '".$branch."' AND `year` = '".$_POST['year']."' AND `season` = '".$_POST['season']."' AND `type` = '团课'";
...
//    echo $_POST['year'];
//    echo $_POST['month'];
    $query = "DELETE FROM `plans` WHERE `branch` = '".$branch."' AND `year` = '".$_POST['year']."' AND `season` = '".$_POST['season']."' AND `type` = '团课'";
    $conn->query($query);
    $conn->close();
    ...
    echo <<<JUMP
            <script language="JavaScript">
                   alert("删除成功");
                   self.location='classTable.php?branch={$branch}';
            </script>
JUMP;
