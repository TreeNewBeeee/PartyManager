	...
	<div class="new-wrap">
		<div class="top-title">
			<p>
				...
				<span class="top-t">新增组织生活会开展情况-<?php echo $branch?></span>
			</p>
		</div>
    <div id="container">
        <div id="content">
...
$path = "../Files/".date("Y")."/".date("m")."/League/Living/";   // 文件存储路径
if (!is_dir($path)){
...
if (isset($_POST['year'])){
    // 上传文件
    if ($_FILES['file']['name'] != '') {
        $filename = "JB-Living-".date("Y-m")."-".$branch."-".$_FILES['file']['name'];    // 重命名文件
...
//        echo "<script>alert('请上传文件！');</script>";
    ...
    $query = "INSERT INTO `plans` (`ID`,`branch`, `year`, `month`, `season`, `fileName`, `type`) 
                  VALUES 
                  (NULL, '".$branch."', '".$_POST['year']."', NULL , '".$_POST['season']."', '".$path.$filename."', '组织生活会')";
    $conn->query($query);
    $conn->close();
