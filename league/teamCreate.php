...
$path = "../Files/".date("Y")."/".date("m")."/League/Team/";   // 文件存储路径
if (!is_dir($path)){
...
if (isset($_POST['year'])){
    // 上传文件
    if ($_FILES['file']['name'] != '') {
        $filename = "JB-Team-".date("Y-m")."-".$branch."-".$_FILES['file']['name'];    // 重命名文件
...
//        echo "<script>alert('请上传文件！');</script>";
    ...
    $query = "INSERT INTO `plans` (`ID`,`branch`, `year`, `month`, `season`, `fileName`, `type`) 
                  VALUES 
                  (NULL, '".$branch."', '".$_POST['year']."', NULL , '".$_POST['season']."', '".$path.$filename."', '团小组会')";
    $conn->query($query);
    $conn->close();
