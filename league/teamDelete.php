	...
	<div class="new-wrap">
		<div class="top-title">
			<p>
				...
				<span class="top-t">删除团小组会开展情况-<?php echo $branch?></span>
			</p>
		</div>
    <div id="container">
        <div id="content">
...
if (isset($_POST['year'])){
    ...
    $query = "SELECT * FROM `plans` WHERE `branch` = '".$branch."' AND `year` = '".$_POST['year']."' AND `season` = '".$_POST['season']."' AND `type` = '团小组会'";
...
//    echo $_POST['year'];
//    echo $_POST['month'];
    $query = "DELETE FROM `plans` WHERE `branch` = '".$branch."' AND `year` = '".$_POST['year']."' AND `season` = '".$_POST['season']."' AND `type` = '团小组会'";
    $conn->query($query);
    $conn->close();
    ...
    echo <<<JUMP
            <script language="JavaScript">
                   alert("删除成功");
                   self.location='teamTable.php?branch={$branch}';
            </script>
JUMP;
