	...
	<div class="new-wrap">
		<div class="top-title">
			<p>
				...
				<span class="top-t">新增青春航迹-<?php echo $branch?></span>
			</p>
		</div>
    <div id="container">
       <div id="content">
...
$path = "../Files/".date("Y")."/".date("m")."/League/YouthTrace/";   // 文件存储路径
if (!is_dir($path)){
...
if (isset($_POST['year'])){
    // 上传文件
    if ($_FILES['file']['name'] != '') {
        $filename = "JB-YouthTrace-".date("Y-m")."-".$branch."-".$_FILES['file']['name'];    // 重命名文件
...
//        echo "<script>alert('请上传文件！');</script>";
    ...
    $query = "INSERT INTO `plans` (`ID`,`branch`, `year`, `month`, `season`, `fileName`, `type`) 
                  VALUES 
                  (NULL, '".$branch."', '".$_POST['year']."', '".$_POST['month']."' , null, '".$path.$filename."', '青春航迹')";
    $conn->query($query);
    $conn->close();
