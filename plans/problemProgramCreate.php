    ...
    <div class="new-wrap">
        <div class="top-title">
            <p>
                ...
                <span class="top-t">新增问题导向性项目-<?php echo $branch ?></span>
            </p>
        </div>
        <div id="container">
            <div id="content">
                    ...
                    <div class="col-md-12">
                        <form method="post" enctype="multipart/form-data" class="layui-form new-form clearfix">
                            <div class="d-info clearfix">
                                <div class="fo-item">
                                    ...
                                    <div class="layui-input-inline">
                                        ...
                                        <input type="text" class="layui-input" id="year" name="year"
                                               placeholder="请填写四位年份如：<?php echo date("Y") ?>">
                                    </div>
                                </div>
                                <div class="fo-item">
                                    <label for="month" class="layui-form-label">项目归属</label>
                                    <div class="layui-input-inline">
                                        <select id="month" name="month">
                                            <option value="支部书记">支部书记</option>
                                            <option value="支部主任">支部主任</option>
                                ...
                                <div class="fo-item">
                                    <label for="title" class="layui-form-label">项目名称</label>
                                    <div class="layui-input-inline">
...
$path = "../Files/" . date("Y") . "/" . date("m") . "/ProblemProgram/";   // 文件存储路径
if (!is_dir($path)) {
...
if (isset($_POST['year'])) {
    // 上传文件
    if ($_FILES['file']['name'] != '') {
        $filename = "JB-问题导向性项目-" . date("Y-m") . "-" . $branch . "-" . $_FILES['file']['name'];    // 重命名文件
...
//        echo "<script>alert('请上传文件！');</script>";
    ...
    $query = "INSERT INTO `plans` (`ID`,`branch`, `year`, `month`, `season`, `fileName`, `type`) 
                  VALUES 
                  (NULL, '" . $branch . "', '" . $_POST['year'] . "', '" . $_POST['month'] . "', '".$_POST['title']."', '" . $path . $filename . "', '问题导向性项目')";
    $conn->query($query);
    $conn->close();
